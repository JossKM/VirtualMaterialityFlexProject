...
void NvFlow::Scene::initDeferred(IRHICommandContext* RHICmdCtx)
{
	...
	bool multiAdapterSafe = !(FlowGridSceneProxy->FlowGridProperties->bParticlesInteractionEnabled || FlowGridSceneProxy->FlowGridProperties->bParticleModeEnabled);

	bool multiAdapterEnabled = multiAdapterSafe && FlowGridSceneProxy->FlowGridProperties->bMultiAdapterEnabled;
	bool asyncComputeEnabled = multiAdapterSafe && FlowGridSceneProxy->FlowGridProperties->bAsyncComputeEnabled;
	...
	if (multiAdapterSafe && UFlowGridAsset::sGlobalMultiGPU > 1)
	{
		m_multiAdapter = m_context->m_multiGPUActive;
	}
	if (multiAdapterSafe && UFlowGridAsset::sGlobalAsyncCompute > 1)
	{
		m_asyncCompute = !m_multiAdapter && m_context->m_asyncComputeActive;
	}
	if (multiAdapterSafe && UFlowGridAsset::sGlobalAsyncCompute > 2 && UFlowGridAsset::sGlobalAsyncCompute > UFlowGridAsset::sGlobalMultiGPU)
	{
